...
`/architectural-analysis` dispatches `structural-analyst`, `behavioral-analyst`, `concurrency-analyst`, `risk-analyst`, and `software-architect` (five agents). `system-architect` is dispatched separately (by you directly or by `/plan-implementation`) when the work crosses a service or bounded-context seam.
