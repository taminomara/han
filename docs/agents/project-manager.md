...
The agent has two modes — facilitation (live discussion) and synthesis (final plan after discussion). Invoke the right one for the situation.
...
- A set of specialist findings is on the table (from UX, DevOps, security, architecture, testing, etc.) and someone needs to reconcile the recommendations and produce a coherent plan the team can commit to.
