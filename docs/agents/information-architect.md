...
- **Name the top tasks.** JTBD statements ("when I install the plugin, I want to run my first skill, so I can decide whether to keep it") focus the audit on the paths readers actually take.
...
The agent's protocols and vocabulary are grounded in published IA and technical-communication frameworks. Each source below is cited because the agent draws specific, named artifacts from it — not as a reading list, but as the provenance of the principles the agent applies.
