...
- **Driving constraint:** The behavioral spec is settled and just cleared `iterative-plan-review`; the skill it extends is freshly reconciled around the `audit`/`spike`/no-output item types (commits `7d8d1a4`, `74980fc`, `71e085c`, last 90 days) and is the baseline to build on before that reconciliation drifts. The prior over-scoped attempt at `docs/plans/autonomous-driver-overview/` is the rejected precedent this plan deliberately stays smaller than.
