...
Current state: `/home/taminomara/p/han/docs/plans/autonomous-driver-per-item-skill-selection/feature-specification.md` and its `/home/taminomara/p/han/docs/plans/autonomous-driver-per-item-skill-selection/artifacts/decision-log.md`. The artifact under review.
...
Full analysis written to: `/home/taminomara/p/han/docs/plans/autonomous-driver-per-item-skill-selection/artifacts/gap-analysis-vs-core-loop.md`
...
- **Current State:** `autonomous-driver-per-item-skill-selection/feature-specification.md` Out of Scope: "No companion changes to `tdd`, `code-review`, `skill-builder`, and so on; the producer records fields and the driver dispatches by instruction, mirroring how the core loop imposed its dispatch contract." Coordinations table: "The driver companion change preserves the core loop's fail-closed startup validation and one-commit-per-item model" (citing D1). `autonomous-driver-per-item-skill-selection/artifacts/decision-log.md` D1 Rationale: "Making the driver read the fields keeps the choice honest end to end." No statement confirms that the `tdd`-specific evidence requirement (test-failure-then-pass) still applies when `tdd` is dispatched via the recorded field rather than hardcoded.
