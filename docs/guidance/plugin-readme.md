...
The README may include sections for Skills, Custom Agents, Hooks, MCP, and LSP — but only include sections for entity types the plugin actually provides. Every plugin has at least one skill, so the Skills section is always present. Don't include empty sections.
...
2. Only include entity sections (Skills, Agents, Hooks, MCP, LSP) that the plugin actually provides
