...
- **Three modes.** Creating new, Converting existing (e.g., an ADR into a standard), Updating existing.
...
- **Hierarchically-prefixed filenames.** `{top-level}[-{second-level}]-{hyphenated-name}.md` — a one- or two-level hierarchy prefix (e.g., `svelte-stores-state-shape.md`) discovered at runtime from existing standards and project context, so related standards sort together in a directory listing.
...
2. **A source document, optional.** If you want to convert an existing document (e.g., an ADR) into a standard, pass the path.
