...
- **What it does.** Takes a `work-items.md` file produced by [`/plan-work-items`](./plan-work-items.md), validates the format, and creates one Jira ticket per work item in a single target project through the Atlassian MCP server.
...
- **Posting to GitHub.** Use [`/work-items-to-issues`](./work-items-to-issues.md) to create GitHub issues instead.
...
- **Run [`/plan-work-items`](./plan-work-items.md) upstream.** This skill publishes a work-items file; it does not produce one. A sharp, dependency-ordered breakdown makes the create step clean.
