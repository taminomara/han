...
9. **Verify.** Step 9.0 runs the self-consistency check (extract `{task-id, file-path, line-range, recommended-action-summary}` tuples, then compare overlapping pairs and demote contradictory recommendations with a `Tension with {other-task-id}:` note). Step 9.1 then verifies task IDs are sequential, `file_path:line_number` references are valid, exploit fields are populated for security findings, the summary table matches the detail sections, and the YAGNI section's verbatim opening is preserved.
