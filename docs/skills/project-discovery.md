...
- **Only discovered data.** Empty sections are omitted entirely — no "{framework}" placeholders, no invented build commands. If a command wasn't actually found, it doesn't appear.
...
- **Pair with `/architectural-decision-record`** when the discovery surfaces an implicit decision (e.g., "we are on PostgreSQL because…") that was never recorded.
...
5. **Verification.** Spot-check a few discovered paths with Glob, confirm no placeholders remain, report results to the user.
