...
- **Item types and no-output work.** Each item records a `Type`. A `deliverable` builds and commits as today. An `audit` runs checks and confirms a result; when it declares `Expected paths: None` it completes as a distinct `done-no-commit` outcome rather than forcing an empty commit. A `spike` records a finding at its Expected path. The driver validates `Type` at startup and refuses the marker combinations that would route a broken or no-op build onto the no-output path.
