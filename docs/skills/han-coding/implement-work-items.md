...
- **Item types and no-output work.** Each item records a `Type`. A `deliverable` builds and commits as today. An `audit` runs checks and confirms a result; when it declares `Expected paths: None` it completes as a distinct `done-no-commit` outcome rather than forcing an empty commit. A `spike` records a finding at the path in its `Expected paths`. The driver validates `Type` at startup and refuses the marker combinations that would route a broken or no-op build onto the no-output path.
...
- **Halt into a recovery menu.** The first item the driver cannot finish cleanly halts, leaving the completed items committed and the halting item's work in the tree. A needs-a-human item is no longer a reason it cannot finish; those are driven now. Every halt uses one five-part frame: status line, one-sentence reason, tree-state disclosure, supporting evidence, and a recovery menu: fix the item in place and re-attempt it, run more automated fix rounds, skip it (the driver names the dependents the skip strands first), or stop the run so a later invocation on the same file resumes it.
...
1. **A path to a `work-items.md`.** The file `/plan-work-items` produced. Every item must carry an `Expected paths` block plus the three per-item fields (`Requires pre-work decisions`, `Suggested implementation`, `Suggested review`), the build and review each tagged `AFK` or `HITL`, or recorded as a bare `none`. The driver runs one whole-run startup check that confirms every item is **drivable**: each item's recorded implementation skill resolves to an installed, invocable skill (or is a bare `none` free-form build), and its review is one the driver can turn into a verdict or a human read. It aborts the whole run before branching when a named implementation skill is neither dispatchable nor invocable, or when an item's markers are invalid: an unrecognized `Type`, `Expected paths: None` on a non-`audit` item, an `AFK` review on an `audit` or no-output item, or a `` `none`, AFK `` build. A missing `Type` reads as `deliverable`, so a file written before this feature drives unchanged.
...
- **A completion summary** naming the branch, each item's execution mode and outcome (committed with its commit reference or range, a no-output `audit` recorded as `done-no-commit`, or the one item that halted the run and why), the items not reached, and your next action (review and push the branch).
- **A durable review record per item** at `.implement-work-items/reviews/<W-N>-iter<fix-round>.md` (one per review round), plus an uncommitted work-state file at `.implement-work-items/state.json`. Both live in the driver's `.implement-work-items/` directory, which ignores everything but the committed `progress.md` ledger, so they never enter a commit and never show in `git status`. The review record holds the full findings; the condensed verdict the driver gates on references it by task ID.
- **On a halt, the five-part frame** instead of a summary of success: which item and phase stopped the run, the one-sentence reason, the tree state, the supporting evidence (with a pointer to the review record when the halt is review-gated), and a recovery menu (fix in place and re-attempt, run more automated fix rounds, skip the item, or stop the run). Stopping is resumable: re-invoking on the same work-items file picks the run back up from the first unfinished item after you confirm.
...
`/implement-work-items` ships a deliberately small slice of a larger driver design: the decide, build, verify, review, fix, commit loop, in-session pauses for the human, cross-session resume from a committed progress ledger, an in-session recovery menu (fix and re-attempt, more fix rounds, skip, or stop resumably), and one shared re-grounding routine on every re-entry. Still deferred, each with a named reopening trigger in the feature's plan: automatic mid-run compaction survival through an external trigger or hook, concurrent drivers on one checkout, detecting edits to an already-done item, the richer recovery menu (gate override, upstream repair, on-demand triage, accept-a-flagged-change), and end-of-run history cleanup. The driver's answer to any state it cannot resolve safely is the fail-closed halt, now surfacing that recovery menu rather than a dead end.
...
The run **resumes across sessions**. Alongside the current session's uncommitted work-state file, the driver keeps a durable progress ledger it commits to the run's branch (`.implement-work-items/progress.md`), recording each item as it starts and finishes. Re-invoking on the same work-items file reads that ledger, reconstructs which items are done, in progress, or skipped, and, after you give a go-ahead, resumes from the first unfinished item rather than rebuilding from the top. The in-session pauses this run takes for a decision, a foreground build, or a human review still happen within a session; the ledger is what carries a partially-complete run forward to the next one. Startup no longer refuses its own prior-run branch: it resumes it. A branch that carries commits but holds no record for this file is a foreign base, and startup still refuses that.
...
The **halt surfaces a recovery menu**. The larger driver design offers a richer menu still (on-demand triage, accept a flagged change, override the gate, repair an upstream item); the driver offers the safe core of it: fix the item in place and re-attempt, run more automated fix rounds, skip the item (its stranded dependents named first), or stop the run and resume it later. It halts on the first item it cannot finish unattended and reports where and why, on the stated assumption that you would rather steer a clean stop than have the driver guess. A verification command that *reports failures* enters the fix loop; a command that *fails to execute* (a missing tool, a downed service) is a tooling-or-environment halt, never fed to the fix loop as if the code were at fault. A finding that a fix round cannot resolve because it needs a human decision (the scope or approach must change for the feature to work or be secure, an unforeseen architectural problem, an unresolvable RAID item) is escalated by the build report or the review verdict and halts for the decision, rather than churning fix rounds against an item no unattended edit can settle. No review sees known-broken code: a red verify routes straight to a fix.
