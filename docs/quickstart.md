...
> See also: [Plugin landing page — han](../README.md) · [Concepts](./concepts.md) · [Skills](./skills/README.md) · [Agents](./agents/README.md) · [Sizing](./sizing.md) · [YAGNI](./yagni.md)
...
1. **[`/investigate`](./skills/investigate.md)** — Evidence-based investigation: file paths, line numbers, git history, test coverage. Produces an investigation report with root cause and a proposed fix that an `adversarial-validator` has already tried to falsify.
...
3. **[`/coding-standard`](./skills/coding-standard.md)** *(as needed)* — Formalize coding conventions — from existing patterns, or from research. `/code-review` checks these automatically.
