...
Han ships as three plugins in one marketplace. Two of them carry components; the third is a convenience wrapper.
...
- **`han.github`.** The GitHub layer. It adds two skills that talk to GitHub through the `gh` CLI: [`gh-pr-review`](./skills/gh-pr-review.md), which posts a code review as comments on a pull request, and [`update-pr-description`](./skills/update-pr-description.md), which writes a PR description from the branch's changes. This plugin depends on `han.core`.
...
`han.github` carries only two skills, so you might expect installing it to give you a GitHub-only slice of Han. It does not.
...
That means **there is no GitHub-only install.** The real choice is only two options wide:
