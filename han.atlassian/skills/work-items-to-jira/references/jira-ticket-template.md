...
- **Issue type ← the resolved type.** Defaults to `Story`. The user may override per run; the chosen type must exist in the target project's issue-type metadata.
- **Parent ← the epic (optional).** When the user names an epic, set the new ticket's parent to that epic's key so the ticket appears under the epic. Use the field the project's create metadata exposes for epic membership (modern projects use `parent`; some company-managed projects use a legacy "Epic Link" field — surface this if `parent` is rejected).
