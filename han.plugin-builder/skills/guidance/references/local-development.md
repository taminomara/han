...
If you previously installed `your-org/your-marketplace` from GitHub, remove it so it doesn't conflict with the local source:

1. Run `/plugin`
2. Switch to the **Marketplaces** tab
3. Select `your-org/your-marketplace` and remove it
...
When you're done testing, remove the local marketplace and re-add the remote `your-org/your-marketplace` source to go back to the published versions.

## Quicker alternatives

The local-marketplace setup above is the canonical local-marketplace workflow because it mirrors how users install your plugin. For quick, throwaway iteration, Claude Code also supports two lighter approaches documented in the official [Create Plugins](https://code.claude.com/docs/en/plugins) guide:

- **`claude --plugin-dir ./example-plugin`** loads a single plugin directory for one session with no marketplace or install step. Useful for a fast check of one plugin's changes.
