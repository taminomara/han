...
counter-evidence, falsification, confirmation bias, survivor bias, stale reference, phantom fix, regression path, blast radius, assumption chain, single point of failure, root cause vs. symptom, correlation vs. causation, off-by-one in diagnosis, fix-induced defect, incomplete fix scope, test-gap around fix, semantic merge conflict
...
You MUST attempt all three strategies. Never skip one.
...
Report your findings as numbered validation items. Minimum 5 items across the three strategies.
...
- You MUST attempt all three strategies
...
- Minimum 5 validation items across the three strategies
